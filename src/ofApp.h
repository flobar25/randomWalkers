...
public:
    Walker(float boundaryX, float boundaryY, float startX, float startY, ofColor color, float speedFactor){
        ...
        this->velocityX = ofRandom(-2, 2) * speedFactor;
        this->velocityY = ofRandom(-2, 2) *  speedFactor;
    ...
    void update(){
        ...
        velocityX += (ofNoise(lastX * 0.005, lastY * 0.005) - 0.5) * 2.0 * speedFactor;
        velocityY += abs((ofNoise(lastY * 0.005, lastX * 0.005) - 0.5) * 2.0) * speedFactor;
        lastX += velocityX;
        lastY += velocityY;
